...
//        System.out.println(json.toString());
    ...
    public Object getLoanRepaymentSchedule(@PathVariable String loanAccount) throws JSONException, JsonProcessingException {
        ...
        for (int i = 1; i < jsonLoanArray.length(); i++) {
            ...
            String amountDue = jsonLoanSummary.getString("totalDueForPeriod");
            String amountPaid = jsonLoanSummary.getString("totalPaidForPeriod");
            String amountOutstanding = jsonLoanSummary.getString("totalOutstandingForPeriod");
...
//            musoniService.disbursementSms(loan_id, phone_number, timestamps);
    ...
    //A function that returns all disbursements by date range and group by month
    @GetMapping("loans/disbursed-by-range/{fromDate}/{toDate}")
    public List<DisbursedLoanMonth> getLoansDisbursedByDateRange(@PathVariable String fromDate, @PathVariable String toDate) {
